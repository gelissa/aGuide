...
function changeletters() {
	...
	achange("t", "7");
	achange("z", "2");
	if (transtype == 1 && !document.getElementById("adv").checked) return;
	achange("d", "|)");
	achange("f", "|=");
	achange("h", "|-|");
	achange("ll", "|_|_");
...
function tol33t() {
	transtype = 1;
	textin = " " + document.getElementById("lametext").value + " ";
	textin = textin.toLowerCase();
	punct();
	changewords();
	changeletters();
	unpunct();
	document.getElementById("l33ttext").value = stripspaces(randomcase(textin));
}

function tolame() {
	transtype = 2;
	textin = " " + document.getElementById("l33ttext").value + " ";
	textin = textin.toLowerCase();
	...
	changewords();
	unpunct();
	document.getElementById("lametext").value = stripspaces(textin);
}
...
function lamechanged() {
	if (1 == 1) tol33t();
}

function l33tchanged() {
	if (1 == 1) tolame();
}
...
function betastatus() {
	document.write("Pretty reliable. No doubt more reliable than the old one. But I'm still working on it to eliminate as many errors as possible.");
}
